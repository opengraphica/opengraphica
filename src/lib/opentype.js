var opentype = (() => {
  ...
  function parseVheaTable(data, start) {
    ...
    vhea.ascent = p.parseShort();
    vhea.vertTypoAscender = vhea.ascent;
    vhea.descent = p.parseShort();
    vhea.vertTypoDescender = vhea.descent;
    vhea.lineGap = p.parseShort();
    vhea.vertTypoLineGap = vhea.lineGap;
  ...
  var vhea_default = { parse: parseVheaTable, make: makeVheaTable };
  ...
  function makeVmtxTable(glyphs) {
    const t = new table_default.Table("vmtx", []);
    for (let i = 0; i < glyphs.length; i += 1) {
      const glyph = glyphs.get(i);
  ...
  var vmtx_default = { parse: parseVmtxTable, make: makeVmtxTable };
  ...
  function parseBuffer(buffer, opt = {}) {
    ...
    if (vmtxTableEntry) {
      ...
      vmtx_default.parse(font, vmtxTable.data, vmtxTable.offset, font.numOfLongVerMetrics, font.numGlyphs, font.glyphs, opt);
    }
