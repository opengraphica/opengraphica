...
export async function openFromFileDialog(options: FileDialogOpenOptions = {}): Promise<void> {
    ...
    // Peasant old Javascript manual labor.
        ...
        fileInput.type = 'file';
        fileInput.setAttribute('accept', options.accept || '.json,.ora,image/*,video/*');
...
export async function openFromFileList({ files, dialogOptions }: FileListOpenOptions = {}) {
    ...
    type FileReadResolve =
        ...
        | { type: 'ora', result: ArrayBuffer, file: File }
        ...
        readerPromises.push(
            new Promise(async (resolveReader, rejectReader) => {
                try {
                    ...
                    let fileReadType: FileReadType = 'media';
                    if (file.type === 'text/plain' || file.name.match(/\.json$/)) {
                        fileReadType = 'json';
                    } else if (file.type === 'image/openraster') {
                        fileReadType = 'ora';
                    }
                    ...
                    else if (fileReadType === 'ora') {
                        fileReader.onload = async () => {
                            if (dialogOptions?.cancelRef?.value === true) {
                                rejectReader('File open canceled.');
                            } else {
                                resolveReader({ type: 'ora', result: fileReader.result as ArrayBuffer, file: file });
                            }
                        };
                        fileReader.onerror = () => {
                            rejectReader('An error occurred while reading the file "' + file.name + '".');
    ...
    for (let readerSettle of settledReaderPromises) {
        if (readerSettle.status === 'fulfilled') {
            ...
            if (readerSettle.value.type === 'image') {
            ...
            else if (readerSettle.value.type === 'ora') {
                const { parseSerializedFileToActions } = await import('@/modules/file/formats/ora-open');
                try {
                    const { workingFileDefinition, insertLayerActions: workingFileInsertLayerActions }
                        = await parseSerializedFileToActions(
                            new Uint8Array(readerSettle.value.result),
                            { insert: dialogOptions.insert }
                        );
                ...
                const { parseSerializedFileToActions } = await import('@/modules/file/formats/json-open');
                const { workingFileDefinition, insertLayerActions: workingFileInsertLayerActions }
                    = await parseSerializedFileToActions(JSON.parse(readerSettle.value.result));
                for (let prop in workingFileDefinition) {
                    if (prop === 'width') {
                        newFileOptions.width = Math.max(newFileOptions.width || 1, workingFileDefinition.width || 1);
                    } else if (prop === 'height') {
