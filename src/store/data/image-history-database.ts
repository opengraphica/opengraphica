...
type DatabaseImageData = string | HTMLImageElement | ImageData | Blob;

interface MemoryStore {
    isMemory: true;
    images: { [key: string]: DatabaseImageData };
}
...
export default {
            ...
            databaseInitPromise = new Promise<void>(async (resolveInit) => {
                try {
                    if (window.indexedDB) {
                        ...
                        try {
                            let lastDatabaseTabPing = localStorage.getItem('openGraphica_historyUsagePing');
     ...
     */
    async get<T = DatabaseImageData>(imageId: string): Promise<T> {
        await this.init();
        if (database?.isMemory) {
