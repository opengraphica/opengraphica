...
export default class Text implements Drawable<TextData> {
    ...
    private async loadFonts(document: TextDocument): Promise<boolean> {
        ...
        for (const subset of subsets) {
            for (const defaultFontFamily of defaultFontFamilies) {
                if (defaultFontFamily.subsets.includes(subset)) {
    ...
    private splitSpanByAvailableFonts(span: TextDocumentSpan): Array<[Font | null, TextDocumentSpan]> {
            ...
            if (document.boundary === 'box') {
                ...
                for (const line of document.lines) {
                    for (const fullSpan of line.spans) {
                        const spanFontSplits = this.splitSpanByAvailableFonts(fullSpan);
