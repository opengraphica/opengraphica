...
import { getWebgl2RendererBackend, markRenderDirty, requestFrontendVideoFrame } from '@/renderers/webgl2/backend';
import { messageBus, type MessageBusEvents } from '@/renderers/webgl2/backend/message-bus';
...
import type {
    ...
    WorkingFileLayerBlendingMode, WorkingFileVideoLayer, WorkingFileLayerFilter
} from '@/types';

export class VideoLayerMeshController implements Webgl2RendererMeshController {
    ...
    attach(id: number) {
        ...
        messageBus.on('frontend.replyFrontendVideoFrame', this.handleVideoFrameCallback);
    ...
    async updateData(data: WorkingFileVideoLayer['data']) {
        ...
        messageBus.emit('backend.requestFrontendVideoFrame', this.sourceUuid);
    }
    ...
    handleVideoFrameCallback(event?: MessageBusEvents['frontend.replyFrontendVideoFrame']) {
        if (!event?.bitmap || !this.sourceUuid || this.sourceUuid !== event.sourceUuid || !this.sourceCanvasContext || !this.sourceTexture) return;
        this.sourceCanvasContext.transferFromImageBitmap(event.bitmap);
    ...
    detach() {
        ...
        messageBus.off('frontend.replyFrontendVideoFrame', this.handleVideoFrameCallback);
