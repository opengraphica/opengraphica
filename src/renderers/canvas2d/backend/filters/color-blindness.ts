...
import { ColorBlindnessSimulationMethod, ColorBlindnessType, type ColorBlindnessCanvasFilterParams } from '@/canvas/filters/color-blindness';
...
export function fragment(
    filter: CanvasFilter<ColorBlindnessCanvasFilterParams>,
    sourceImageData: Uint8ClampedArray,
    targetImageData: Uint8ClampedArray,
    dataPosition: number,
) {
    const type = filter.params.type ?? 0;
    const method = filter.params.method ?? 0;
    const severity = filter.params.severity ?? 1;
