export function transfer8BitImageDataToSrgb(imageData: Uint8ClampedArray, dataPosition: number): { r: number, g: number, b: number, alpha: number } {
    return {
        r: imageData[dataPosition + 0] / 255,
        g: imageData[dataPosition + 1] / 255,
        b: imageData[dataPosition + 2] / 255,
        alpha: imageData[dataPosition + 3] / 255
    }
}

export function transfer8BitImageDataToLinearSrgb(imageData: Uint8ClampedArray, dataPosition: number): { r: number, g: number, b: number, alpha: number } {
    return {
...
export function transferSrgbTo8BitImageData(rgba: { r: number, g: number, b: number, alpha: number }, imageData: Uint8ClampedArray, dataPosition: number) {
    imageData[dataPosition + 0] = rgba.r * 255;
    imageData[dataPosition + 1] = rgba.g * 255;
    imageData[dataPosition + 2] = rgba.b * 255;
    imageData[dataPosition + 3] = rgba.alpha * 255;
}

export function transferLinearSrgbTo8BitImageData(rgba: { r: number, g: number, b: number, alpha: number }, imageData: Uint8ClampedArray, dataPosition: number) {
