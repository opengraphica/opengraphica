<template>
    ...
    >
        <template v-for="menuDrawer of menuDrawers" :key="menuDrawer.id">
            <section
                :ref="el => { drawerEls[menuDrawer.id] = el as HTMLElement }"
                class="og-menu-drawer"
                :class="['is-open-' + menuDrawer.placement, { 'is-closing': !menuDrawer.visible, 'is-immediate': menuDrawer.immediate }]"
            >
                <h1 class="og-menu-drawer__title">
                    {{ t(menuDrawer.title || 'empty') }}
                </h1>
